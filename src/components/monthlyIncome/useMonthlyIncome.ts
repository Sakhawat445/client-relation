...
export const useMonthlyIncome = () => {
  const customers =
    (useAppSelector((state) => state.customer.customers) as Customer[]) || [];
  ...
  const totalSpendings = Customers.reduce(
    (sum, customer) => sum + (customer.spendings ?? 0),
    0,
  );
